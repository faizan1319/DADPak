...
router.get('/getPostByUserId/:userId', function(req, res) {
	...
	var sql = "SELECT postId, postMediaType, postMediaFileURL, postTitle, postDesc, postLat, postLng, postLikes, postDislikes, postCommentCount, DATEDIFF(CURRENT_DATE(), postedDate) AS dateDiff, TIMEDIFF(CURRENT_TIME(), postedTime) AS timeDiff FROM posts WHERE userId = ? ORDER BY postedDate, postedTime DESC";
	var inserts = [userId];
